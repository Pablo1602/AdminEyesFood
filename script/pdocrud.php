...
function afterLoginCallBack($data, $obj) {
    ...
    //save data in session
        $_SESSION["data"] = $data;
        $obj->formRedirection("https://eyesfoodweb.herokuapp.com/Usuarios/index");
    }
    else{
...
function afterLoginCallBack2($data, $obj) {
    ...
    //save data in session
        $_SESSION["data"] = $data;
        $obj->formRedirection("https://eyesfoodweb.herokuapp.com/Expertos/perfil");
    }
    else{
        //no record found so don't redirect
        $obj->formRedirection("");
...
function afterLoginCallBackTienda($data, $obj) {
    ...
    //save data in session
        $_SESSION["data"] = $data;
        $obj->formRedirection("https://eyesfoodweb.herokuapp.com/Tiendas/index");
    }
    else{
        //no record found so don't redirect
        $obj->formRedirection("");
...
function afterRegisterCallBack($data, $obj) {
    $obj->formRedirection("https://eyesfoodweb.herokuapp.com/Login/registroDone");
}

function afterUpdateCallBack($data, $obj) {
    //$obj->formRedirection(base_url());
    $obj->formRedirection("https://eyesfoodweb.herokuapp.com/Tiendas");
}
